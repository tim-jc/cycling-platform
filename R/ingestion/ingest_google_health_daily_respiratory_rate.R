ingest_google_health_daily_respiratory_rate <- function(
  connection,
  run_id,
  source_id,
  config,
  ...
  run_entity_id <- create_etl_run_entity(
    ...
    run_id = run_id,
    entity_name = "google_health_daily_respiratory_rate"
  )
  ...
  tryCatch(
    {
      for (i in seq_along(date_batches)) {
        ...
        message(glue::glue(
          "Processing Google Health daily respiratory-rate batch ",
        ...
        batch_rows <- purrr::pmap_dfr(
          ...
          \(date, start_datetime, end_datetime) {
            message(glue::glue(
              "Retrieving Google Health daily respiratory rate for {date}."
            ))
            ...
            request_rows <- tryCatch(
              get_google_health_daily_respiratory_rate(
                run_id = run_id,
                source_id = source_id,
        ...
        batch_result <- DBI::dbWithTransaction(
          conn = connection,
          {
            upsert_google_health_daily_respiratory_rate(
              connection = connection,
              daily_respiratory_rate = batch_rows
            )
          }
        )
        ...
        unknown_source_rows <- if (nrow(batch_rows) == 0) {
          0L
        } else {
          sum(
            is.na(batch_rows$source_ecosystem) |
              batch_rows$source_ecosystem == "" |
              batch_rows$source_ecosystem == "unknown"
          )
        }

        message(glue::glue(
          "Completed Google Health daily respiratory-rate batch ",
          "{i}/{length(date_batches)}: ",
          "{batch_result$rows_inserted} inserted, ",
          "{batch_result$rows_updated} updated, ",
          "{unknown_source_rows} unknown-source rows."
        ))
      }
