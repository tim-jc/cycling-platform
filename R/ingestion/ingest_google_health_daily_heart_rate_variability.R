ingest_google_health_daily_heart_rate_variability <- function(
  connection,
  run_id,
  source_id,
  config,
  ...
  run_entity_id <- create_etl_run_entity(
    ...
    run_id = run_id,
    entity_name = "google_health_daily_heart_rate_variability"
  )
  ...
  tryCatch(
    {
      for (i in seq_along(date_batches)) {
        ...
        message(glue::glue(
          "Processing Google Health daily HRV batch ",
        ...
        batch_rows <- purrr::pmap_dfr(
          ...
          \(date, start_datetime, end_datetime) {
            message(glue::glue(
              "Retrieving Google Health daily HRV for {date}."
            ))
            ...
            request_rows <- tryCatch(
              get_google_health_daily_heart_rate_variability(
                run_id = run_id,
                source_id = source_id,
        ...
        batch_result <- DBI::dbWithTransaction(
          conn = connection,
          {
            upsert_google_health_daily_heart_rate_variability(
              connection = connection,
              daily_heart_rate_variability = batch_rows
            )
          }
        )
        ...
        message(glue::glue(
          "Completed Google Health daily HRV batch ",
          "{i}/{length(date_batches)}: ",
