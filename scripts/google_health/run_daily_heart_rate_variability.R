...
if (length(args) == 2) {
  ...
  run_mode <- "MANUAL"
  refresh_days <- config$ingestion$google_health_daily_heart_rate_variability_refresh_days
  ...
  run_mode <- "BACKFILL"
  refresh_days <- config$ingestion$google_health_daily_heart_rate_variability_backfill_days
