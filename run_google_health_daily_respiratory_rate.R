...
if (length(args) == 2) {
...
} else if (execution_mode %in% c("refresh", "manual", "repair")) {
  run_mode <- if (execution_mode == "repair") {
    "REPAIR"
  } else {
    "MANUAL"
  }
  refresh_days <- config$ingestion$google_health_daily_respiratory_rate_refresh_days
  ...
  run_mode <- "BACKFILL"
  refresh_days <- config$ingestion$google_health_daily_respiratory_rate_backfill_days
  ...
  stop(
    paste(
      ...
      "'repair', or provide explicit start_date end_date."
    ),
    call. = FALSE
  )
}
